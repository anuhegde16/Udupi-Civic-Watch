...
export function PanchayatMap({ officers, reports }: PanchayatMapProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      let fitBounds: [[number, number], [number, number]] = FALLBACK_BOUNDS;
  ...
  // Redraw markers/polygons when data changes
  useEffect(() => {
    ...
    (async () => {
      ...
      reports.forEach((report) => {
        ...
        badge.style.cssText = `display:inline-block;font-size:10px;font-weight:800;
          ...
          padding:1px 7px;border-radius:99px;margin-bottom:5px;`;
        badge.textContent =
          report.status === "reported" ? "New"
          : report.status === "cleaning" ? "In Progress"
          : "Cleaned";
        popup.appendChild(badge);
