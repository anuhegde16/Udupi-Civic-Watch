...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  reported: { label: "Reported", color: "text-destructive", bg: "bg-destructive/10", dot: "bg-destructive" },
  cleaning: { label: "Cleaning", color: "text-orange-600", bg: "bg-orange-50", dot: "bg-orange-500" },
  cleaned: { label: "Cleaned", color: "text-primary", bg: "bg-primary/10", dot: "bg-primary" },
};
      ...
      <div className="bg-card rounded-3xl p-6 md:p-8 border border-border/50 shadow-sm relative overflow-hidden">
        ...
        {!isLoadingStats && (
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: "All Time Total", value: stats?.total ?? 0, icon: <LayoutList className="w-5 h-5" />, color: "text-foreground", bg: "bg-muted/60" },
              { label: "New Reports", value: stats?.reported ?? 0, icon: <AlertCircle className="w-5 h-5" />, color: "text-destructive", bg: "bg-destructive/8" },
              { label: "In Progress", value: stats?.cleaning ?? 0, icon: <Clock className="w-5 h-5" />, color: "text-orange-500", bg: "bg-orange-50" },
              { label: "Cleaned", value: stats?.cleaned ?? 0, icon: <CheckCircle2 className="w-5 h-5" />, color: "text-indigo-600", bg: "bg-indigo-50" },
            ].map((s) => (
