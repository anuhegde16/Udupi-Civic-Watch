...
import { Loader2, Search, FileWarning, CheckCircle2, HardHat, MapPin, UserIcon, CalendarIcon, Anchor } from "lucide-react";
    ...
    <div className="pb-12 animate-in fade-in duration-500">
      ...
      {isLoadingReports ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-card rounded-[2.5rem] border border-border/50 border-dashed text-center px-4 shadow-sm">
          ...
          <p className="text-muted-foreground font-medium max-w-md">Try adjusting your filters to find what you're looking for.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-border/50">
                {reports.map((report, i) => (
                  <tr key={report.id} className="hover:bg-muted/30 transition-colors animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${i * 30}ms` }}>
                    ...
                    <td className="px-6 py-5 text-right">
                      ...
                      >
                        {report.assignedOfficer ? 'Reassign' : 'Dispatch Officer'}
                      </Button>
                    </td>
                  </tr>
                ))}
                ...
                {officers.map((off) => (
                  <SelectItem key={off.id} value={off.id.toString()} className="py-3 focus:bg-muted">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground mt-0.5">{off.areaName || 'No specific area'} • <span className={off.pendingCount > 0 ? "text-secondary-foreground font-bold" : ""}>{off.pendingCount} pending</span></span>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
