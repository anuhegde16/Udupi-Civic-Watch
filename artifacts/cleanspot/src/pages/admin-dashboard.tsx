...
export default function AdminDashboard() {
  ...
  }, [filteredReports, selectedOfficerId, summary]);
  ...
  const trendData = useMemo(() => {
    if (!selectedOfficerId) return analytics?.dailyTrend || [];
    const now = new Date();
    const buckets: { day: string; date: Date }[] = [];
    for (let i = 13; i >= 0; i--) {
